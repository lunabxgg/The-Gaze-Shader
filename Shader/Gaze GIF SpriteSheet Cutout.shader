Shader "@Luna/Gaze GIF SpriteSheet Cutout"
{
    // ========================================
    // Gaze GIF Cutout为了不透明的图片的简易Shader,保留了VRC光体积
    ...
    Properties
    {
        ...
        [IntRange] _Columns ("Columns", Range(1, 64)) = 4
        [IntRange] _Rows ("Rows", Range(1, 64)) = 4
        [IntRange] _TotalFrames ("Total Frames", Range(1, 4096)) = 16
        
        [Header(GIF Conversion Source)]
        [NoScaleOffset] _GifTexture ("GIF Source (Internal)", 2D) = "black" {}
        ...
        [NoScaleOffset]_NormalMap ("Normal Map SpriteSheet", 2D) = "bump" {}
    ...
    SubShader
    {
        ...
        // 全局安全宏与函数声明
        // ==========================================
        inline float hash(float n) { return frac(sin(n) * 43758.5453123); }
        inline float hash(float2 p) { return frac(sin(dot(p, float2(12.9898, 78.233))) * 43758.5453); }
        ...
        float GenerateObjectSeed(float3 worldPos) { return frac(sin(dot(worldPos, float3(12.9898, 78.233, 45.543))) * 43758.5453); }
        float RandomFromSeed(float seed, float offset) { return frac(sin(dot(float2(seed, offset), float2(12.9898, 78.233))) * 43758.5453); }
        ...
        float _Columns, _Rows, _TotalFrames;
        ...
        // 精灵图计算与光照函数
        // ==========================================
        float2 GetSpriteSheetUV(float2 uv, uint frame) 
        {
            float2 frameSize = float2(1.0 / _Columns, 1.0 / _Rows);
            uint col = frame % (uint)_Columns;
            uint row = (uint)_Rows - 1 - (frame / (uint)_Columns);
            float2 offset = float2(col, row) * frameSize;
            return uv * frameSize + offset;
        }
        ...
        fixed4 fragCommon (v2f i, bool isBackface)
        {
            ...
            // 精灵图特有的单格 UV 映射（带防溢出保护）
            float2 sheetUV = GetSpriteSheetUV(frac(uv), frame);
            fixed4 col = tex2D(_MainTex, sheetUV);
            col.rgb *= _Color.rgb;
            ...
            #if defined(_NORMAL_MAP)
            float3 n = UnpackNormal(tex2D(_NormalMap, sheetUV));
            n.xy *= _NormalStrength;
            // 通过 XY 重构完美的 Z 轴深度
            n.z = sqrt(max(0.001, 1.0 - dot(n.xy, n.xy)));
            n = normalize(n);
            float3 finalNormal = normalize(mul(float3x3(normalize(i.worldTangent), normalize(i.worldBitangent), normalize(i.worldNormal)), n));
            #else
